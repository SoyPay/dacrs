...
public:
	...
	bool IsEmptyValue() const {
		return !(llValues > 0 || !vFreedomFund.empty());
	}
...
public:
	CAppFundOperate();
	unsigned char vTag[CAppCFund::MAX_TAG_SIZE + 1];	//!< accountid
	...
	const vector<unsigned char> GetTagV() const {

		char len = vTag[0];
		assert(len <= CAppCFund::MAX_TAG_SIZE);
		vector<unsigned char> tag(&vTag[1], &vTag[len + 1]);
		return (tag);

	}
	CAppFundOperate(const vector<unsigned char> &tag, APP_OP_TYPE opType,int timeout,int64_t money)
	{
		assert(CAppCFund::MAX_TAG_SIZE >= tag.size());
		vTag[0]= tag.size();
		memcpy(&vTag[1],&tag[0],vTag[0]);
		mMoney = money;
		outheight = timeout;
		opeatortype = opType;
	}
