...
void CScriptDBTest::GetScriptData(CScriptDBViewCache* pViewCache) {
	...
	bool ret = pViewCache->GetScriptData(nCurHeight, CRegID(it->first), 0, vScriptKey, vScriptData, nHeight,
			setOperLog);
	...
	while (ret) {
		...
		ret = pViewCache->GetScriptData(nCurHeight, CRegID(it->first), 1, vScriptKey, vScriptData, nHeight, setOperLog);
