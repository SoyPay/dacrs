// Copyright (c) 2012-2013 The Bitcoin Core developers
// Distributed under the MIT/X11 software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#include "bloom.h"

#include "base58.h"
#include "key.h"
#include "main.h"
#include "serialize.h"
#include "uint256.h"
#include "util.h"

#include <vector>

#include <boost/test/unit_test.hpp>

using namespace std;
using namespace boost::tuples;

BOOST_AUTO_TEST_SUITE(bloom_tests)

BOOST_AUTO_TEST_CASE(bloom_create_insert_serialize)
{
	CBloomFilter filter(3, 0.01, 0, EM_BLOOM_UPDATE_ALL);

	filter.insert(ParseHex("99108ad8ed9bb6274d3980bab5a85c048f0950c8"));
	BOOST_CHECK_MESSAGE( filter.contains(ParseHex("99108ad8ed9bb6274d3980bab5a85c048f0950c8")), "BloomFilter doesn't contain just-inserted object!");
	// One bit different in first byte
	BOOST_CHECK_MESSAGE(!filter.contains(ParseHex("19108ad8ed9bb6274d3980bab5a85c048f0950c8")), "BloomFilter contains something it shouldn't!");

	filter.insert(ParseHex("b5a2c786d9ef4658287ced5914b37a1b4aa32eee"));
	BOOST_CHECK_MESSAGE(filter.contains(ParseHex("b5a2c786d9ef4658287ced5914b37a1b4aa32eee")), "BloomFilter doesn't contain just-inserted object (2)!");

	filter.insert(ParseHex("b9300670b4c5366e95b2699e8b18bc75e5f729c5"));
	BOOST_CHECK_MESSAGE(filter.contains(ParseHex("b9300670b4c5366e95b2699e8b18bc75e5f729c5")), "BloomFilter doesn't contain just-inserted object (3)!");

	CDataStream stream(SER_NETWORK, g_sProtocolVersion);
	filter.Serialize(stream, SER_NETWORK, g_sProtocolVersion);

	vector<unsigned char> vch = ParseHex("03614e9b050000000000000001");
	vector<char> expected(vch.size());

	for (unsigned int i = 0; i < vch.size(); i++)
	expected[i] = (char)vch[i];

	BOOST_CHECK_EQUAL_COLLECTIONS(stream.begin(), stream.end(), expected.begin(), expected.end());
}

BOOST_AUTO_TEST_CASE(bloom_create_insert_serialize_with_tweak)
{
	// Same test as bloom_create_insert_serialize, but we add a nTweak of 100
	CBloomFilter filter(3, 0.01, 2147483649, EM_BLOOM_UPDATE_ALL);

	filter.insert(ParseHex("99108ad8ed9bb6274d3980bab5a85c048f0950c8"));
	BOOST_CHECK_MESSAGE( filter.contains(ParseHex("99108ad8ed9bb6274d3980bab5a85c048f0950c8")), "BloomFilter doesn't contain just-inserted object!");
	// One bit different in first byte
	BOOST_CHECK_MESSAGE(!filter.contains(ParseHex("19108ad8ed9bb6274d3980bab5a85c048f0950c8")), "BloomFilter contains something it shouldn't!");

	filter.insert(ParseHex("b5a2c786d9ef4658287ced5914b37a1b4aa32eee"));
	BOOST_CHECK_MESSAGE(filter.contains(ParseHex("b5a2c786d9ef4658287ced5914b37a1b4aa32eee")), "BloomFilter doesn't contain just-inserted object (2)!");

	filter.insert(ParseHex("b9300670b4c5366e95b2699e8b18bc75e5f729c5"));
	BOOST_CHECK_MESSAGE(filter.contains(ParseHex("b9300670b4c5366e95b2699e8b18bc75e5f729c5")), "BloomFilter doesn't contain just-inserted object (3)!");

	CDataStream stream(SER_NETWORK, g_sProtocolVersion);
	filter.Serialize(stream, SER_NETWORK, g_sProtocolVersion);

	vector<unsigned char> vch = ParseHex("03ce4299050000000100008001");
	vector<char> expected(vch.size());

	for (unsigned int i = 0; i < vch.size(); i++)
	expected[i] = (char)vch[i];

	BOOST_CHECK_EQUAL_COLLECTIONS(stream.begin(), stream.end(), expected.begin(), expected.end());
}

BOOST_AUTO_TEST_CASE(bloom_create_insert_key)
{

	if (SysCfg().emNetWork::EM_TESTNET == SysCfg().NetworkID() || SysCfg().emNetWork::EM_REGTEST == SysCfg().NetworkID()) {
		return ;
	}
	string strSecret = string("5Kg1gnAjaLfKiwhhPpGS3QfRg2m6awQvaj98JCZBZQ5SuS2F15C");
	CDacrsSecret vchSecret;
	BOOST_CHECK(vchSecret.SetString(strSecret));

	CKey key = vchSecret.GetKey();
	CPubKey pubkey = key.GetPubKey();
	vector<unsigned char> vchPubKey(pubkey.begin(), pubkey.end());

	CBloomFilter filter(2, 0.001, 0, EM_BLOOM_UPDATE_ALL);
	filter.insert(vchPubKey);
	uint160 hash = pubkey.GetKeyID();
	filter.insert(vector<unsigned char>(hash.begin(), hash.end()));

	CDataStream stream(SER_NETWORK, g_sProtocolVersion);
	filter.Serialize(stream, SER_NETWORK, g_sProtocolVersion);

	vector<unsigned char> vch = ParseHex("038fc16b080000000000000001");
	vector<char> expected(vch.size());

	for (unsigned int i = 0; i < vch.size(); i++)
	expected[i] = (char)vch[i];

	BOOST_CHECK_EQUAL_COLLECTIONS(stream.begin(), stream.end(), expected.begin(), expected.end());
}

BOOST_AUTO_TEST_CASE(merkle_block_1)
{
	// Random real block (34ef2048a4c15e5a76e55af5cdbac0f0bd008a51ca68d233d7cb39158cb21933)
	// With 9 txes
	CBlock block;
	
	CDataStream stream(ParseHex("020000004d0449f6a390f734568e3ad4bead7b4abd8cdfe48839\
ce8151b50ef2ec3f0debcd37f5d3b75a2f58ef560459bf11c29070eee1c2\
64b9cffc3c436b7a8a621a9976746c2e752750315464708766f359a6b69e\
c40e3f843ff2b56fd0cb6b0400002754325882661c1bb701000066040000\
00000000000000001b010000473045022100b2a5e1ad39ae2eeaafbff026\
ea510362ac5e6cc253e36f969679ebc8b4e2f28102203aa8aa2bb33328ba\
ed172124f9274e8bf61d7b62b175625d27f878b5adc1287603010202000d\
82dbebaf2087660302876502000d140031f37de36553a23438846f342ac3\
bd765fe1f5cd109399febadda9000046304402206aaa631b6dca6e004bc8\
536acfdc75008fa93e132e53f499b2ecfdcaa7ec1fa602200c6d0fcabad4\
47b81f513eabec6cfc04294bf65576317665563ba20a766e950a03028765\
02000d140031f37de36553a23438846f342ac3bd765fe1f5cd108181d8ad\
8fef0000473045022100fc89572a8152d9294b6dd1c2834882e04bb62134\
6af41ea41c6a2e6107df545a02207b640c21116b01878bc1b6b58b94ec3a\
c38bd52715bd94bc7505c28ffdb4b0d0"), SER_NETWORK, g_sProtocolVersion);
	//CDataStream stream(ParseHex(""), SER_NETWORK, PROTOCOL_VERSION);
	stream >> block;

	CBloomFilter filter(10, 0.000001, 0, EM_BLOOM_UPDATE_ALL);
	// Match the last transaction
	//filter.insert(uint256S("0xfbadf1682488087592fb8f48c2b3cb22a23273409d64bbb3e35469c6e9125da3"));
	filter.insert(uint256S("0xb09135750f39e4ccb81c39ace2820f8eabecf0ee8e4138427091130ffe45c13c"));
	CMerkleBlock merkleBlock(block, filter);
	BOOST_CHECK(merkleBlock.m_cBlockHeader.GetHash() == block.GetHash());

	BOOST_CHECK(merkleBlock.m_vMatchedTxn.size() == 1);
	pair<unsigned int, uint256> pair = merkleBlock.m_vMatchedTxn[0];

	BOOST_CHECK(merkleBlock.m_vMatchedTxn[0].second == uint256S("0xb09135750f39e4ccb81c39ace2820f8eabecf0ee8e4138427091130ffe45c13c"));
	BOOST_CHECK(merkleBlock.m_vMatchedTxn[0].first == 1);

	vector<uint256> vMatched;
	BOOST_CHECK(merkleBlock.m_cTxNum.ExtractMatches(vMatched) == block.GetHashMerkleRoot());
	BOOST_CHECK(vMatched.size() == merkleBlock.m_vMatchedTxn.size());
	for (unsigned int i = 0; i < vMatched.size(); i++)
	BOOST_CHECK(vMatched[i] == merkleBlock.m_vMatchedTxn[i].second);

	// Also match the 8th transaction
	filter.insert(uint256S("0x9eeed094d4dc4327ae5af54ba92b991ae545840d2fa882eb90c0f62d0885f5b9"));
	merkleBlock = CMerkleBlock(block, filter);
	BOOST_CHECK(merkleBlock.m_cBlockHeader.GetHash() == block.GetHash());

	BOOST_CHECK(merkleBlock.m_vMatchedTxn.size() == 2);

	BOOST_CHECK(merkleBlock.m_vMatchedTxn[0] == pair);

	BOOST_CHECK(merkleBlock.m_vMatchedTxn[1].second == uint256S("0x9eeed094d4dc4327ae5af54ba92b991ae545840d2fa882eb90c0f62d0885f5b9"));
	BOOST_CHECK(merkleBlock.m_vMatchedTxn[1].first == 2);

	BOOST_CHECK(merkleBlock.m_cTxNum.ExtractMatches(vMatched) == block.GetHashMerkleRoot());
	BOOST_CHECK(vMatched.size() == merkleBlock.m_vMatchedTxn.size());
	for (unsigned int i = 0; i < vMatched.size(); i++)
	BOOST_CHECK(vMatched[i] == merkleBlock.m_vMatchedTxn[i].second);
}

BOOST_AUTO_TEST_CASE(merkle_block_2)
{
	// Random real block (c6c28d39d04e8ed6b92850fab5f214490e62697890eab9f1b393a8805da397f6)
	// With 4 txes
	CBlock block;
	CDataStream stream(ParseHex("020000004d0449f6a390f734568e3ad4bead7b4abd8cdfe48839\
ce8151b50ef2ec3f0debcd37f5d3b75a2f58ef560459bf11c29070eee1c2\
64b9cffc3c436b7a8a621a9976746c2e752750315464708766f359a6b69e\
c40e3f843ff2b56fd0cb6b0400002754325882661c1bb701000066040000\
00000000000000001b010000473045022100b2a5e1ad39ae2eeaafbff026\
ea510362ac5e6cc253e36f969679ebc8b4e2f28102203aa8aa2bb33328ba\
ed172124f9274e8bf61d7b62b175625d27f878b5adc1287603010202000d\
82dbebaf2087660302876502000d140031f37de36553a23438846f342ac3\
bd765fe1f5cd109399febadda9000046304402206aaa631b6dca6e004bc8\
536acfdc75008fa93e132e53f499b2ecfdcaa7ec1fa602200c6d0fcabad4\
47b81f513eabec6cfc04294bf65576317665563ba20a766e950a03028765\
02000d140031f37de36553a23438846f342ac3bd765fe1f5cd108181d8ad\
8fef0000473045022100fc89572a8152d9294b6dd1c2834882e04bb62134\
6af41ea41c6a2e6107df545a02207b640c21116b01878bc1b6b58b94ec3a\
c38bd52715bd94bc7505c28ffdb4b0d0"), SER_NETWORK, g_sProtocolVersion);
	//CDataStream stream(ParseHex(""), SER_NETWORK, PROTOCOL_VERSION);
	stream >> block;

	CBloomFilter filter(10, 0.000001, 0, EM_BLOOM_UPDATE_ALL);
	// Match the first transaction
	filter.insert(uint256S("0xed525c5920c8f353a1d6a080f5f6e1c3bb12fa5100efe8cce44ed6f04682b61b"));

	CMerkleBlock merkleBlock(block, filter);
	BOOST_CHECK(merkleBlock.m_cBlockHeader.GetHash() == block.GetHash());

	BOOST_CHECK(merkleBlock.m_vMatchedTxn.size() == 1);

	BOOST_CHECK(merkleBlock.m_vMatchedTxn[0].second == uint256S("0xed525c5920c8f353a1d6a080f5f6e1c3bb12fa5100efe8cce44ed6f04682b61b"));
	BOOST_CHECK(merkleBlock.m_vMatchedTxn[0].first == 0);

	vector<uint256> vMatched;
	BOOST_CHECK(merkleBlock.m_cTxNum.ExtractMatches(vMatched) == block.GetHashMerkleRoot());
	BOOST_CHECK(vMatched.size() == merkleBlock.m_vMatchedTxn.size());
	for (unsigned int i = 0; i < vMatched.size(); i++)
	BOOST_CHECK(vMatched[i] == merkleBlock.m_vMatchedTxn[i].second);
}

BOOST_AUTO_TEST_CASE(merkle_block_2_with_update_none)
{
	// Random real block (c6c28d39d04e8ed6b92850fab5f214490e62697890eab9f1b393a8805da397f6)
	// With 4 txes
	CBlock block;
		CDataStream stream(ParseHex("020000004d0449f6a390f734568e3ad4bead7b4abd8cdfe48839\
ce8151b50ef2ec3f0debcd37f5d3b75a2f58ef560459bf11c29070eee1c2\
64b9cffc3c436b7a8a621a9976746c2e752750315464708766f359a6b69e\
c40e3f843ff2b56fd0cb6b0400002754325882661c1bb701000066040000\
00000000000000001b010000473045022100b2a5e1ad39ae2eeaafbff026\
ea510362ac5e6cc253e36f969679ebc8b4e2f28102203aa8aa2bb33328ba\
ed172124f9274e8bf61d7b62b175625d27f878b5adc1287603010202000d\
82dbebaf2087660302876502000d140031f37de36553a23438846f342ac3\
bd765fe1f5cd109399febadda9000046304402206aaa631b6dca6e004bc8\
536acfdc75008fa93e132e53f499b2ecfdcaa7ec1fa602200c6d0fcabad4\
47b81f513eabec6cfc04294bf65576317665563ba20a766e950a03028765\
02000d140031f37de36553a23438846f342ac3bd765fe1f5cd108181d8ad\
8fef0000473045022100fc89572a8152d9294b6dd1c2834882e04bb62134\
6af41ea41c6a2e6107df545a02207b640c21116b01878bc1b6b58b94ec3a\
c38bd52715bd94bc7505c28ffdb4b0d0"), SER_NETWORK, g_sProtocolVersion);
	//CDataStream stream(ParseHex(""), SER_NETWORK, PROTOCOL_VERSION);
	stream >> block;

	CBloomFilter filter(10, 0.000001, 0, EM_BLOOM_UPDATE_NONE);
	// Match the first transaction
	filter.insert(uint256S("0xed525c5920c8f353a1d6a080f5f6e1c3bb12fa5100efe8cce44ed6f04682b61b"));

	CMerkleBlock merkleBlock(block, filter);
	BOOST_CHECK(merkleBlock.m_cBlockHeader.GetHash() == block.GetHash());

	BOOST_CHECK(merkleBlock.m_vMatchedTxn.size() == 1);

	BOOST_CHECK(merkleBlock.m_vMatchedTxn[0].second == uint256S("0xed525c5920c8f353a1d6a080f5f6e1c3bb12fa5100efe8cce44ed6f04682b61b"));
	BOOST_CHECK(merkleBlock.m_vMatchedTxn[0].first == 0);

	vector<uint256> vMatched;
	BOOST_CHECK(merkleBlock.m_cTxNum.ExtractMatches(vMatched) == block.GetHashMerkleRoot());
	BOOST_CHECK(vMatched.size() == merkleBlock.m_vMatchedTxn.size());
	for (unsigned int i = 0; i < vMatched.size(); i++)
	BOOST_CHECK(vMatched[i] == merkleBlock.m_vMatchedTxn[i].second);

}

BOOST_AUTO_TEST_CASE(merkle_block_3_and_serialize)
{
	// Random real block (cd0529322ed0393491966d4adb13141775dcc5a1408f495ede95e14b41185262)
	// With one tx
	CBlock block;
	CDataStream stream(ParseHex("0200000060203f9559534438ca3e15b94a9b0d304d3b62ad264a\
dc87ac62a30b8753712a175f70e4113da4ecc3df18f55458acee8c3d4f0a\
cc47a2416ece8189c22d343c9f9d3321322c3a5f275d6532250d49405406\
8fa74eb9fcc6b05696ddb4060000c154325839ec181b2f0200006c040000\
0000000000000000940000004630440220363ceed0b99a6fd24c80f7d88a\
5f437a81bc92de454d50e6a8af65984c850de702203693ebc10c3c92d027\
e7ac79057100ccb3a6b5f30f18a197e4993cd40cd520c801010202000782\
dbea9300876c"), SER_NETWORK, g_sProtocolVersion);//CDataStream stream(ParseHex(""), SER_NETWORK, PROTOCOL_VERSION);
	stream >> block;

	CBloomFilter filter(10, 0.000001, 0, EM_BLOOM_UPDATE_ALL);
	// Match the only transaction
	filter.insert(uint256S("0x3c342dc28981ce6e41a247cc0a4f3d8ceeac5854f518dfc3eca43d11e4705f17"));

	CMerkleBlock merkleBlock(block, filter);
	BOOST_CHECK(merkleBlock.m_cBlockHeader.GetHash() == block.GetHash());

	BOOST_CHECK(merkleBlock.m_vMatchedTxn.size() == 1);

	BOOST_CHECK(merkleBlock.m_vMatchedTxn[0].second == uint256S("0x3c342dc28981ce6e41a247cc0a4f3d8ceeac5854f518dfc3eca43d11e4705f17"));
	BOOST_CHECK(merkleBlock.m_vMatchedTxn[0].first == 0);

	vector<uint256> vMatched;
	BOOST_CHECK(merkleBlock.m_cTxNum.ExtractMatches(vMatched) == block.GetHashMerkleRoot());
	BOOST_CHECK(vMatched.size() == merkleBlock.m_vMatchedTxn.size());
	for (unsigned int i = 0; i < vMatched.size(); i++)
	BOOST_CHECK(vMatched[i] == merkleBlock.m_vMatchedTxn[i].second);

	CDataStream merkleStream(SER_NETWORK, g_sProtocolVersion);
	merkleStream << merkleBlock;
//	cout << "MerkleBlock:" << HexStr(merkleStream) << endl;
	//Hex: BlockHeader + txnum + txhash + bits
	vector<unsigned char> vch = ParseHex("");//ParseHex("");

	vector<char> expected(vch.size());

	for (unsigned int i = 0; i < vch.size(); i++)
	expected[i] = (char)vch[i];

	BOOST_CHECK_EQUAL_COLLECTIONS(expected.begin(), expected.end(), merkleStream.begin(), merkleStream.end());
}

BOOST_AUTO_TEST_CASE(merkle_block_4)
{
	// Random real block (6ea6fda338a47a0c57fd3bce48a7c40d3b106c7506b9b7e44005683b89ceda7d)
	// With 7 txes
	CBlock block;
	CDataStream stream(ParseHex("020000004d0449f6a390f734568e3ad4bead7b4abd8cdfe48839\
			ce8151b50ef2ec3f0debcd37f5d3b75a2f58ef560459bf11c29070eee1c2\
			64b9cffc3c436b7a8a621a9976746c2e752750315464708766f359a6b69e\
			c40e3f843ff2b56fd0cb6b0400002754325882661c1bb701000066040000\
			00000000000000001b010000473045022100b2a5e1ad39ae2eeaafbff026\
			ea510362ac5e6cc253e36f969679ebc8b4e2f28102203aa8aa2bb33328ba\
			ed172124f9274e8bf61d7b62b175625d27f878b5adc1287603010202000d\
			82dbebaf2087660302876502000d140031f37de36553a23438846f342ac3\
			bd765fe1f5cd109399febadda9000046304402206aaa631b6dca6e004bc8\
			536acfdc75008fa93e132e53f499b2ecfdcaa7ec1fa602200c6d0fcabad4\
			47b81f513eabec6cfc04294bf65576317665563ba20a766e950a03028765\
			02000d140031f37de36553a23438846f342ac3bd765fe1f5cd108181d8ad\
			8fef0000473045022100fc89572a8152d9294b6dd1c2834882e04bb62134\
			6af41ea41c6a2e6107df545a02207b640c21116b01878bc1b6b58b94ec3a\
			c38bd52715bd94bc7505c28ffdb4b0d0"), SER_NETWORK, g_sProtocolVersion);//CDataStream stream(ParseHex(""), SER_NETWORK, PROTOCOL_VERSION);
	stream >> block;

	CBloomFilter filter(10, 0.000001, 0, EM_BLOOM_UPDATE_ALL);
	// Match the last transaction
	filter.insert(uint256S("0x9eeed094d4dc4327ae5af54ba92b991ae545840d2fa882eb90c0f62d0885f5b9"));

	CMerkleBlock merkleBlock(block, filter);
	BOOST_CHECK(merkleBlock.m_cBlockHeader.GetHash() == block.GetHash());

	BOOST_CHECK(merkleBlock.m_vMatchedTxn.size() == 1);
	pair<unsigned int, uint256> pair = merkleBlock.m_vMatchedTxn[0];

	BOOST_CHECK(merkleBlock.m_vMatchedTxn[0].second == uint256S("0x9eeed094d4dc4327ae5af54ba92b991ae545840d2fa882eb90c0f62d0885f5b9"));
	BOOST_CHECK(merkleBlock.m_vMatchedTxn[0].first == 2);

	vector<uint256> vMatched;
	BOOST_CHECK(merkleBlock.m_cTxNum.ExtractMatches(vMatched) == block.GetHashMerkleRoot());
	BOOST_CHECK(vMatched.size() == merkleBlock.m_vMatchedTxn.size());
	for (unsigned int i = 0; i < vMatched.size(); i++)
	BOOST_CHECK(vMatched[i] == merkleBlock.m_vMatchedTxn[i].second);

	// Also match the 4th transaction
	filter.insert(uint256S("0xb09135750f39e4ccb81c39ace2820f8eabecf0ee8e4138427091130ffe45c13c"));
	merkleBlock = CMerkleBlock(block, filter);
	BOOST_CHECK(merkleBlock.m_cBlockHeader.GetHash() == block.GetHash());

	BOOST_CHECK(merkleBlock.m_vMatchedTxn.size() == 2);

	BOOST_CHECK(merkleBlock.m_vMatchedTxn[0].second == uint256S("0xb09135750f39e4ccb81c39ace2820f8eabecf0ee8e4138427091130ffe45c13c"));
	BOOST_CHECK(merkleBlock.m_vMatchedTxn[0].first == 1);

	BOOST_CHECK(merkleBlock.m_vMatchedTxn[1] == pair);

	BOOST_CHECK(merkleBlock.m_cTxNum.ExtractMatches(vMatched) == block.GetHashMerkleRoot());
	BOOST_CHECK(vMatched.size() == merkleBlock.m_vMatchedTxn.size());
	for (unsigned int i = 0; i < vMatched.size(); i++)
	BOOST_CHECK(vMatched[i] == merkleBlock.m_vMatchedTxn[i].second);
}

BOOST_AUTO_TEST_SUITE_END()
