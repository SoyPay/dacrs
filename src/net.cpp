...
CAddress GetLocalAddress(const CNetAddr *paddrPeer)
{
    ...
    {
        ret = CAddress(addr);
        ret.nServices = g_ullLocalServices;
...
bool AddLocal(const CService& addr, int nScore)
{
    ...
    {
        LOCK(g_cs_mapLocalHost);
        bool fAlready = g_mapLocalHost.count(addr) > 0;
        LocalServiceInfo &info = g_mapLocalHost[addr];
...
void SetLimited(enum Network net, bool fLimited)
{
    if (net == NET_UNROUTABLE)
        return;
    LOCK(g_cs_mapLocalHost);
    g_sLimited[net] = fLimited;
}

bool IsLimited(enum Network net)
{
    LOCK(g_cs_mapLocalHost);
    return g_sLimited[net];
}
...
bool SeenLocal(const CService& addr)
{
    {
        LOCK(g_cs_mapLocalHost);
        if (g_mapLocalHost.count(addr) == 0)
            return false;
        g_mapLocalHost[addr].nScore++;
    }
...
{
    LOCK(g_cs_mapLocalHost);
    return g_mapLocalHost.count(addr) > 0;
}
...
//    const char* pszGet;
    ...
    for (int nHost = 1; nHost <= 2; nHost++)
    {
        ...
        if (GetMyExternalIP2(addrConnect, strszGet.c_str(), pszKeyword, cIpRet))
            return true;
    }
...
void CNode::PushVersion()
{
    ...
    LogPrint("net", "send version message: version %d, blocks=%d, us=%s, them=%s, peer=%s\n", g_sProtocolVersion, nBestHeight, addrMe.ToString(), addrYou.ToString(), addr.ToString());
    PushMessage("version", g_sProtocolVersion, g_ullLocalServices, nTime, addrYou, addrMe,
    		g_ullLocalHostNonce, FormatSubVersion(CLIENT_NAME, g_sClientVersion, comments), nBestHeight, true);
}
...
void CNode::copyStats(CNodeStats &stats)
{
    ...
    X(nSendBytes);
    X(nRecvBytes);
    stats.fSyncNode = (this == g_sNodeSync);
...
void ThreadSocketHandler()
{
    ...
    while (true)
    {
        ...
        if(vNodes.size() != nPrevNodeCount) {
            ...
            g_cUIInterface.NotifyNumConnectionsChanged(nPrevNodeCount);
            g_cUIInterface.NotifyMessage(strprintf("connections=%d", nPrevNodeCount));
        }
...
void ThreadDNSAddressSeed() {
	...
	for (const auto &seed : vcSeeds) {
		if (HaveNameProxy()) {
			...
			addrman.Add(vcAdd, CNetAddr(seed.strName, true));
		}
	}
...
void ThreadOpenAddedConnections()
{
    ...
    for (unsigned int i = 0; true; i++)
    {
        ...
        // (keeping in mind that addnode entries can have many IPs if g_bNameLookup)
        {
            LOCK(cs_vNodes);
            for (auto pnode : vNodes)
...
void RelayTransaction(CBaseTransaction *pBaseTx, const uint256& cHash, const CDataStream& cDS) {
	CInv inv(MSG_TX, cHash);
	{
		...
		mapRelay.insert(make_pair(inv, cDS));
	...
	for (auto pnode : vNodes) {
		...
		if (pnode->pfilter) {
			if (pnode->pfilter->IsRelevantAndUpdate(pBaseTx, cHash)) {
