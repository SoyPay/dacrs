...
Object TxToJSON(CBaseTransaction *pTx) {
	...
	case CONTRACT_TX: {
		...
		result.push_back(Pair("ver", prtx->nVersion));
		result.push_back(Pair("script id", HexStr(prtx->scriptRegId)));
		{
			Array array;
		...
		for (int ii = 0; ii < tx.vSignature.size(); ii++) {
			...
			if (!view.GetAccount(tx.vAccountRegId.at(ii), account)) {
				throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY,
						string("unregister RegID: ") + HexStr(tx.vAccountRegId.at(ii)));
			}
...
Value registerscripttx(const Array& params, bool fHelp) {
	...
	if (params.size() > 4) {
		RPCTypeCheck(params, list_of(str_type)(str_type)(int_type)(int_type)(int_type));
		nAuthorizeTime = params[4].get_uint64();
	}
	if (params.size() > 5) {
	...
	if (params.size() > 8) {
		RPCTypeCheck(params, list_of(str_type)(str_type)(int_type)(int_type)(int_type)(int_type)(int_type)(int_type)(int_type));
		nUserDefine = params[8].get_int();
	}
	...
	CRegistScriptTx tx;
	{
		...
		tx.aAuthorizate.SetUserDefine(nUserDefine);
...
Value getaddrfrozendetail(const Array& params, bool fHelp) {
	if (fHelp || params.size() != 1) {
		string msg = "registeraccounttx nrequired [\"key\",...] ( \"account\" )\n"
				"\nAdd a nrequired-to-sign multisignature address to the wallet.\n"
				"Each key is a Bitcoin address or hex-encoded public key.\n" + HelpExampleCli("registeraccounttx", "")
				+ "\nAs json rpc call\n" + HelpExampleRpc("registeraccounttx", "");
		throw runtime_error(msg);
	}
